...
import sys, os, math, enum, pprint, time
...
def dist(v0, v1):
    return math.sqrt((v0.coords[0]-v1.coords[0])**2 + (v0.coords[1]-v1.coords[1])**2 + (v0.coords[2]-v1.coords[2])**2)
...
def buildTriangles( slice0, slice1 ):
    ...
    # Initialize array of minimum distance
    closestPair = []

    # Before going through all vertices in slices, assume first vertices pair have shortest 
    # distance until proven otherwise
    closestPair = [slice0.verts[0], slice1.verts[0]]
    closestDist = dist(slice0.verts[0], slice1.verts[1])

    # Iterate through all vertices combinations to determine shortest distance
    for vertex0 in slice0.verts:
        for vertex1 in slice1.verts:
            if dist(vertex0, vertex1) < closestDist:
                closestDist = dist(vertex0, vertex1)
                closestPair = [vertex0, vertex1]
    ...
    # Determine the positions of the vertices in the closest pair in their respective arrays
    index0 = slice0.verts.index(closestPair[0])
    index1 = slice1.verts.index(closestPair[1])

    # Create cyclic permutations that begin after the closest pair, then continue from
    # the beginning until the closest pair, then include the closest pair vertices at the end 
    # using slicing in Python
    cyclicPerm0 = []
    cyclicPerm1 = []

    for i in slice0.verts[index0:]:
        cyclicPerm0.append(i)
    for j in slice0.verts[:index0]:
        cyclicPerm0.append(j)
    cyclicPerm0.append(closestPair[0])
    

    for k in slice1.verts[index1:]:
        cyclicPerm1.append(k)
    for l in slice1.verts[:index1]:
        cyclicPerm1.append(l)
    cyclicPerm1.append(closestPair[1])
    ...
    # Create (temporarily) empty arrays for minArea and minDir with dimensions slice1 length x slice2 length
    minDir = []
    minArea = []
    lengthPerm0 = len(cyclicPerm0)
    lengthPerm1 = len(cyclicPerm1)

    for r in range(lengthPerm1):
        minDir.append([])
        minArea.append([])
        for c in range(lengthPerm0):
            minDir[r].append([])
            minArea[r].append([])


    # Initialize starting edge values
    minArea[0][0] = 0 # Starting edge has zero area
    minDir[0][0] = "__" # Will store Dir.PREV_ROW if last min-area triangulation ending in [r][c] 
    # came from previous row, and Dir.PREV_COL if it came from previous column. [0][0] is neither, so store "_".
    ...
    tempAreaC = 0
    for c in range(1, len(slice0.verts)):
        minDir[0][c] = Dir.PREV_ROW
        tempAreaC = triangleArea(cyclicPerm1[0].coords, cyclicPerm0[c].coords, cyclicPerm0[c-1].coords)
        minArea[0][c] = minArea[0][c-1] + tempAreaC
    ...
    tempAreaR = 0
    for r in range(1, len(slice1.verts)):
        minDir[r][0] = Dir.PREV_COL
        tempAreaR = triangleArea(cyclicPerm0[0].coords, cyclicPerm1[r-1].coords, cyclicPerm1[r].coords)
        minArea[r][0] = minArea[r-1][0] + tempAreaR
    ...
    for x in range(lengthPerm1):
        for y in range(lengthPerm0):
            # Use equation for area given in class to calculate area of next row and column
            rowArea = minArea[x-1][y] + triangleArea(cyclicPerm0[y].coords, cyclicPerm1[x].coords, cyclicPerm1[x-1].coords)
            colArea = minArea[x][y-1] + triangleArea(cyclicPerm1[x].coords, cyclicPerm0[y].coords, cyclicPerm0[y-1].coords)
            if min(rowArea, colArea) == rowArea:
                minArea[x][y] = rowArea
                minDir[x][y] = Dir.PREV_COL
            elif min(rowArea, colArea) == colArea:
                minArea[x][y] = colArea
                minDir[x][y] = Dir.PREV_ROW
    ...
    triIndex = 0 # Index for triangles array when building triangulation
    rowIndex = lengthPerm1 - 1 # Max index of rows
    colIndex = lengthPerm0 - 1 # Max index of columns
    while (1):
        # End loop when coordinates reach (0,0)
        if minDir[rowIndex][colIndex] == "__":
            break
        else:
            if minDir[rowIndex][colIndex] == Dir.PREV_ROW:
                # If the direction of minimum area is the previous row, triangulate within row
                triangles[triIndex] = Triangle([cyclicPerm0[colIndex], cyclicPerm1[rowIndex], cyclicPerm0[colIndex-1]])
                colIndex = colIndex - 1
            elif minDir[rowIndex][colIndex] == Dir.PREV_COL:
                # If the direction of minimum area is the previous column, triangulate within column
                triangles[triIndex] = Triangle([cyclicPerm0[colIndex], cyclicPerm1[rowIndex], cyclicPerm1[rowIndex-1]])
                rowIndex = rowIndex - 1
            triIndex = triIndex + 1 # Increment index of triangles array
